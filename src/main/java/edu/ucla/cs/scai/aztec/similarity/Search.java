...
//    }
    ...
    public ArrayList<String> SearchFunction(String qs) throws Exception{
        // It may be better to distinguish different search attribute ..... serach in author and index the entries
        // the word with - connecting with each other need to be normilized
        ArrayList<String> res_id = new ArrayList<>();
        ArrayList<WeightedEntry> res = new ArrayList<>();
        LinkedList<String> origintokens = new LinkedList<>();
//        LinkedList<RankedString> tokens = new LinkedList<>();
        System.out.println("Search for query"+qs);
...
//        }
//        System.out.println("Parsed query: "+String.join(" + ", origintokens));
        origintokens = textparser.queryParser(qs);
...
//            pw.print("\n");
        ...
        System.out.println("Go through every entries to find the most similar ones to query");
        int count = 0;
        queryLength = Math.sqrt(queryLength);
        int succ = 0;
        for (String entry : CachedData.tfidt.keySet()) { // change to TFIDF without expansion
...
//            System.out.println(docLength);
            ...
            double sim = 0;
//            System.out.print("the word socre of qurey in this entry ");
...
//            System.out.print(".\n");
            ...
            if (sim > 0) {
                ...
                System.out.print("added into final result list! with similarity ");
                System.out.println(sim);
                succ+=1;
            }
            if(count%1000 == 0){
                System.out.print(count);
                System.out.println(" tools have been processed!");
                System.out.println("The similarity of tool "+String.valueOf(count)+" is "+String.valueOf(sim));
            }
            count+=1;
        }
        System.out.println("Total "+String.valueOf(succ)+" entries in the list");
        Collections.sort(res);
        System.out.println("Sorted the final results according to similarity score");
        HashSet<String> namelist = new HashSet<>();
        System.out.println("The ids in the list are" );
        int cnt = 0;
        for (WeightedEntry w: res) {
            if(cnt<20) {
                System.out.print(w.weight);
                System.out.print("  ");
                System.out.println(w.entry.getId());
                System.out.println(w.entry.getDescription());
                System.out.println(w.entry.getAuthors());
                cnt++;
            }
            res_id.add(w.entry.getId());
        }
        System.out.println("res_id array:" + res_id.toString());
        return res_id;
    }
    public ArrayList<WeightedEntry> getMostSimilarEntriesOtherAttributeToQuery(String qs,String infile) throws Exception{
...
//            pw.print("\n");
        ...
        queryLength = Math.sqrt(queryLength);
        //search based on tags, authors, titles and institution. Exact match:
        // need to build a author list and institution list, give high priority since the name is not common
        // tag and title can use same process as the description.
...
//            }
    ...
    public static void main(String[] args) throws Exception{
        ...
        ArrayList<String> resid = handle.SearchFunction("protein protein interaction");
//        for(String s: resid){
//            System.out.println(s);
//        }
//        String outfile = "/home/xinxin/AztecData/searchres/tmp.txt";
//        String query = "rna sequencing";
//        ArrayList<WeightedEntry> res = handle.getMostSimilarEntriesToQueryTFIDF(query, " ",30, outfile);
//        for (WeightedEntry r:res){
//            System.out.print(r.weight);
//            System.out.println(r.entry.getDescription());
//        }
//        handle.loadMap();
//        String infile = "/home/xinxin/AztecData/searchres/query";
//        BufferedReader reader = new BufferedReader(new FileReader(infile));
//        String line;
//        Integer count = 1;
//        while((line = reader.readLine()) != null) {
...
//            String outfile = "/home/xinxin/AztecData/searchres/search_Q" + count.toString() + ".txt";
//
//            //ArrayList<WeightedAbs> res = handle.getMostSimilarAbstract(query,10);
//            ArrayList<String> resid = handle.SearchFunction("Li Chen");
//            ArrayList<WeightedEntry> res = handle.getMostSimilarEntriesToQueryTFIDF(query, " ",30, outfile);
//            count+=1;
//            for(String s: resid){
//                System.out.println(s);
