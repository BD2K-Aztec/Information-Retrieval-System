...
//        }
        ...
        //new attributes!!!!!!!!!!!!!!!!!!!!!!!!!
        // add the id attribute text into the system
        for (String entry : tfidt.keySet()) {
            HashMap<String, Double> row = tfidt.get(entry);
            List<RankedString> rankedIdTokens = CachedData.idkeywords.get(entry); // add the id(name, author instution) attribute into tfidf
            double length = 0;
            for (RankedString w : rankedIdTokens) {
                if(!row.containsKey(w.getString())){
                    row.put(w.getString(),w.getRank());
                }
            }
            tfidt.put(entry,row);
        }
